...
1. Finish Phase 7 staging validation: browser smoke tests, custom 404 body, and repeat deployment/update loop.
2. Prepare Phase 8 production deployment using the same manual GitHub Actions workflow with a different GitHub Environment and production domain.
3. Keep staging non-indexable with `NEXT_PUBLIC_SITE_ENV=development`; production must use `NEXT_PUBLIC_SITE_ENV=production`.
4. Test the documented Sanity content update loop once, then treat broader content refresh as a separate future wave.
5. Replace placeholder imagery with approved festival assets in the future content wave.
