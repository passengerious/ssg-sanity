...
3. Integrate Sanity-backed `festivalCity` content into `/kamianets` and `/lviv` routes.
...
| How should `festivalCity` route rendering coexist with existing generic `Page` route rendering? | Medium | Phase 3 GROQ/static route contract decision |
...
| Festival schemas are not yet wired into frontend routes | Medium | Sanity/Next.js agents | Phase 3 must add GROQ queries and replace temporary fallback params |
